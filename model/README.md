...
$m\ddot{x}+c\dot{x}+kx+S(x - \mu \text{sign}(V_{rel})y) = me\omega^2\cos(\omega t) \\
m\ddot{y}+c\dot{y}+ky+S(y + \mu \text{sign}(V_{rel})x) = me\omega^2\sin(\omega t) \\ {} \\
...
$X''(\tau) + 2\zeta X'(\tau) + X(\tau)+\bar{k_c}(1-\frac{1}{\sqrt{X^2 + Y^2}})(X-\mu \text{sign}(V_{rel})Y) = \bar{e} \Omega^2 \cos(\Omega \tau) \\
